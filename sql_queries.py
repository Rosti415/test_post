...
class BlogDB:
    ...
    def create_post(self,title,text,category, image_name):
        ...
        self.cursor.execute(''' INSERT INTO posts(title,text,category_id) VALUES(?,?,?,?)''', [title,text,category_db[0], image_name])
        self.conn.commit()
        self.close()
...
data = cursor.fetchall()#вибираємо все з таблиці
print(data)#виводимо в тнрмінал дані
